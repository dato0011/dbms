...
pub fn map_native_type_to_sqlz(
    ...
    match native_type {
        ...
        name if name == Type::TIME.name() || name == Type::TIMETZ.name() => GenericType::Time,
...
pub fn map_sqlz_to_native_type(generic_type: &GenericType) -> String {
    match generic_type {
        ...
        GenericType::Uuid => Type::UUID.name().to_string(),
        GenericType::Bit(_) => Type::BIT.name().to_string(),
        GenericType::VarBit(_) => Type::VARBIT.name().to_string(),
        ...
        GenericType::Time => Type::TIME.name().to_string(),
